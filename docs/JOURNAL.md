...
## 2026-08-23 — the user's fix for the render size, codified before it was deleted

They built two Parameter Execute DATs in the COMP by hand - `renderchange` watching
`resolutionw resolutionh` on `render1`, `camchange` watching `orthowidth` on `cam1`,
each calling `me.parent().cook()`. Better than what I had proposed, which was a
reconcile at known moments: this is a real callback, fires only when a parameter
actually moves, and needs nothing pressed.

### The first thing to say about it was not "good idea"

`td_build_vision.py` destroys every master-level child not in its keep set. **Both
DATs would have been gone on the next chain run**, along with the diagnosis behind
them. They are generated by the builder now and named in `kept`.

That is worth generalising: in a project where the network is generated, anything
built by hand in it has a lifetime of one rebuild. The useful response to "I built
this in the COMP" is to codify it immediately, not to admire it.

### The root cause, which explains why an expression could never have worked

TouchDesigner builds a parameter's dependencies from what the expression TOUCHED, and
`op('render1').width` is a plain **Python attribute read** rather than a parameter
reference - so there is nothing for it to watch. `op('render1')` returning None
touches less than that. Either way the parameter is never dirtied and the COMP never
learns its numbers are stale.

`op('render1').par.resolutionw` WOULD be tracked. Not adopted: `.width` is the
resolved size and the parameter is not, so a render sized by a fraction or by its
input would report the wrong thing - which is the failure this whole area keeps
producing. The DATs are the honest fix.

### Two changes I made to their version

`cook(force=True)` rather than `cook()`. An operator TouchDesigner does not believe
is dirty may not cook at all, and "nothing knows it is dirty" is precisely the
problem being solved.

And the path is written whether or not the target exists, so a `render1` added later
binds the watcher when it appears. That was the case they originally reported.

### What the layout self-check earned today

The first build placed `camchange` on top of `seg_par_callbacks` and
`appletd/td_layout.py` refused: `master layout has overlapping nodes`. A table that
raises instead of quietly stacking two DATs at one coordinate, which is exactly what
it exists for.

### Verified, and the two corners that are not

`render1` resized to 1600x900: `Renderw`/`Renderh` followed and the `_ty` gain with
them. Untested: a `render1` created after the build - binding happens, firing on
