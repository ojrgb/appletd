...
### 6. Render and camera changes now propagate — DONE 2026-08-23, one corner untested

Two Parameter Execute DATs on the master COMP, the user's design, now generated by
`tools/td_build_vision.py` so a chain run cannot delete them:

    renderchange   watches /project1/render1  resolutionw resolutionh
    camchange      watches /project1/cam1     orthowidth

Both call `me.parent().cook(force=True)`.

**Why an expression alone was not enough.** TouchDesigner builds a parameter's
dependencies from what the expression TOUCHED, and `op('render1').width` is a plain
Python attribute read, not a parameter reference - so there is nothing to watch.
`op('render1')` returning None touches less than that. Either way the parameter is
never dirtied.

`force=True` and not a bare `cook()`: an operator TouchDesigner does not believe is
dirty may not cook at all, and "nothing knows it is dirty" is the entire problem.

**Verified**: `render1` resized to 1600x900, `Renderw`/`Renderh` followed, and the
`_ty` gain with them.

**The corner still untested**, which is the case that started this: a `render1` that
did not exist when the COMP was built. The DAT's path is written whether or not the
target is there, so it binds when one appears - but CREATION is not a value change,
so the first change to the new render's resolution is probably what actually fires
it. Not tested, because testing it means deleting the user's `render1`.

**And one narrower gap.** These watch the resolution PARAMETERS. A render whose size
comes from somewhere else - a fraction, or "use input" - can change `.width` without
`resolutionw` moving, and that would not fire.
