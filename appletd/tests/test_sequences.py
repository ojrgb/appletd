...
from appletd.derive import Params, derive
from appletd.sequences import SEQUENCES, frames, triangle
from appletd.synth import pinching, snapping, synthetic_frame
from appletd.tuning import LATCH_THRESHOLDS
from appletd.types import LandmarkFrame, channel_names, channel_values

# Which channel each sweep drives, and which threshold pair judges it. The NUMBERS
# come from appletd/tuning.py rather than being re-typed here - see that
...
THRESHOLDS = {
    ...
    # The ring and little sweeps are judged against the shared finger-trigger
    # pair, which is the latch they drive. Every sweep is checked against the
    # thresholds of the latch it is FOR, not against one arbitrary pair.
    "ring": ("h0_pinch_ring", *LATCH_THRESHOLDS["Trigger"]),
    "little": ("h0_pinch_little", *LATCH_THRESHOLDS["Trigger"]),
...
def _swept(name: str, cycles: float = 2.0) -> list[float]:
    ...
    return _swept_channel(name, THRESHOLDS[name][0], cycles)


def _swept_channel(name: str, channel: str, cycles: float = 2.0) -> list[float]:
    """Any channel's value over a sweep, for cross-checking one against another."""
...
def test_each_finger_sweep_drives_only_its_own_distance_to_zero(finger: str) -> None:
    """Eight trigger latches share one set of operators, so the wiring is the risk.

    The sweeps are generated by one factory differing only in the target finger.
    If that parameter were ignored - the exact mutation review found surviving
    once already - every sweep would drive the index distance and four latches
    would be verified by a sweep that never touched them. So: the named finger's
    distance must reach zero, and it must be the SMALLEST of the four.
    """
    sweep = {"index": "ramp", "middle": "snap", "ring": "ring", "little": "little"}
    values = {name: min(_swept_channel(sweep[finger], "h0_pinch_" + name))
              for name in ("index", "middle", "ring", "little")}
    assert values[finger] == pytest.approx(0.0, abs=1e-6), (
        "the %s sweep never brought the thumb to the %s tip: %r"
        % (finger, finger, values))
    others = [v for name, v in values.items() if name != finger]
    assert values[finger] < min(others), (
        "the %s sweep did not make %s the closest finger: %r" % (finger, finger,
                                                                 values))
