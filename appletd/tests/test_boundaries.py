...
def _module_scope_imports(path: Path) -> set[str]:
    ...
    Contract: returns a generous token set, not just root modules - for
              `from appletd.td import bootstrap` it yields `appletd`,
              `appletd.td`, `td` and `bootstrap`. Callers intersect it with
    ...
    and both pinned by test_the_detector_itself_catches_a_violation:
      ...
      * root-name-only matching meant `from appletd.td import bootstrap`
        reported `appletd`, which is in no forbidden set, so it passed.
    """
    ...
    def record(dotted: str) -> None:
        ...
        # appletd.td.bootstrap` hides the forbidden name in the middle, and
        # a root/leaf-only version of this passed that case (caught by this
        # test's own `import appletd.td.bootstrap` line).
        found.update(parts)
...
def test_the_detector_itself_catches_a_violation() -> None:
    ...
    td_cases = {
        ...
        "from appletd.td import bootstrap\n": {"td"},  # was reported as `appletd`
        "import appletd.td.bootstrap\n": {"td"},
        "import td\n": {"td"},
        "def f():\n    from .td import bootstrap\n": set(),
    }
...
def test_the_pyobjc_blocker_actually_blocks() -> None:
    ...
    """
    code, output = _run_without_pyobjc("import appletd.engine\n")
...
import appletd.types, appletd.coords, appletd.source
import appletd.td.bootstrap, appletd.td.hands_chop
from appletd.source import FakeSource, InProcessSource, LatestFrameBox
...
from appletd.td import hands_chop
names = hands_chop.channel_names()
values = hands_chop.channel_values(
    __import__("appletd.types", fromlist=["blank_frame"]).blank_frame(),
...
assert "appletd.engine" not in sys.modules, "engine imported at module scope"
print("ok")
""")
