...
because `derive()` simply does not emit them - measured, switching every derive group
off takes `derive_chop` from 171 channels to 0 and the COMP output from 561 to 393.
Nothing is left stale there.
...
RELABELLED = {
    # What a rename is for, kept as the example: a name about the code rather than
...
def _page(comp, name="Hands"):
    ...
    # channels, `temporal` fell to 12 and `latches` to 45, and the COMP output went
    # from 505 channels to 366 - silently, from re-running two builders that are each
    ...
    # this one REMOVES channels, so it does not belong in that list.
...
def main():
    ...
    # The toggles that are NOT in GROUPS: each removes channels rather than adding a
    # capability. They would be invisible in the report above, and `Facekeypoints`
    # gates real cost.
    ...
    # Every toggle that gates a group's cooking: `allowCooking` is an attribute, so
    # it cannot be bound to a parameter and has to be rewritten on change.
    ...
    callbacks.par.pars = " ".join(gating_toggles)
    callbacks.par.valuechange = True
    print("2. cook gating wired through %s (%s)"
