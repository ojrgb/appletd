...
# time. As Python EXPRESSIONS on the Constant CHOPs these cost 0.110 ms every frame -
# thirty expression evaluations per cook to deliver six numbers that change only when
...
def main():
    ...
    # (DESIGN.md 2.11).
    group = comp.op(GROUP)
    group_existed = group is not None
    if group is None:
    ...
    # `both_valid`, each `active AND live`. Liveness is computed THERE, not here, and
    # that is what breaks the dependency cycle between the two groups: otherwise the
    # latch bank reads `active` from temporal while temporal reads `live` from the
    # latch bank. Flat, TouchDesigner tolerates that because the real data
