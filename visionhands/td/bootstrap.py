"""sys.path wiring and engine lifecycle, for use inside TouchDesigner.
    ...
    HOW TO USE IT, minimally: put this in a Text DAT and run it once.
        ...
        import sys
        sys.path.append('/Users/omer/Documents/GitHub/visionhands-touchdesigner')
...
def start(camera_name: str | None = None,
    ...
    if not add_venv_to_path(site_packages):
        raise RuntimeError(
            "visionhands: no site-packages at %s - create the venv with\n"
            "  ~/.pyenv/versions/3.11.9/bin/python3.11 -m venv ~/.venvs/visionhands\n"
...
def stop() -> None:
    ...
    try:
        ...
        print("visionhands: error stopping source: %r" % (exc,))
...
def status() -> str:
    ...
    current = _source
    if current is None:
        return "visionhands: not started"
    try:
        frame = current.latest()            # type: ignore[attr-defined]
        return ("visionhands: running=%s seq=%d age=%.0fms hands=%d errors=%s"
        ...
        return "visionhands: status unavailable (%r)" % (exc,)
