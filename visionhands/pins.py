...
every number that reaches a user comes from. `visionhands/depth.py` owns the model,
`visionhands/sidecar.py` owns the transport.

numpy IS imported at module scope here, unlike everywhere else in the package. The
rule it breaks is real - `visionhands/__init__.py` promises the core layer needs no
